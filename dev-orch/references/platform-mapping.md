...
生命周期路线和状态语义由 `dev-lc` 定义；执行模式和任务图由 `dev-orch/SKILL.md` 定义。本文件只映射宿主能力，不改变两者的所有权边界。
...
- 调度器是根代理，负责将LC路线映射为任务边界、协调执行和收集结果；子代理只执行被分配的责任。
- 子代理修改文件时必须明确拥有路径或责任对象，并被告知不要回退其他人的修改。
- 为 `dev-cr` 创建新的独立专业任务；不得使用 `followup_task`、`send_message` 或等价续接能力把实现 Agent 转为其自身候选的正式评审者。无法分离时保留独立性缺口，不得形成 `REV Approved`。
...
| 续接或纠偏 | `SendMessage` |
| 停止失效分支 | `TaskStop` |
| 外置状态 | `change_get/lifecycle_get/handoff_get/handoff_list/artifact_list` 控制面读取、`work_get/work_list/agent_run_list` 恢复读取及 `work_prepare/work_claim/work_complete/agent_run_bind` 调度写入；不得向调度 Agent 暴露生命周期、交接、失效、归档或晋升写工具 |

Claude 的 `context: fork`、`agent: dev-orch`、工具白名单和斜杠命令只属于适配层，不得写进工具中立核心协议。

## 无子代理宿主

使用 `route-only`：基于当前 `lifecycle_route_ref` 输出执行投影、版本化任务包、依赖、阻塞和授权缺口。没有LC路线时只报告
`lifecycle_route_status: missing` 和恢复条件；不得自行生成权威路线、声称专业阶段已执行或把中间状态写入项目。
