...
在明确版本、环境和授权范围内执行已经批准的后端与系统集成测试，保留可复核证据，区分执行结果、证据有效性和验证结论。
...
- `GATE` 是指定候选和验证范围的版本化验证结论记录，聚合执行、证据、缺陷、未验证范围、例外和剩余风险；它仍只是开发验证门建议，不等于风险接受、外部交付批准或生产稳定。
- `BUILD` 只证明实现阶段本地检查，不自动成为正式 `RUN/EVD`。
- 不执行部署、生产迁移、流量切换、生产数据修复或事故恢复；这些工作属于套件外部责任流程。
- 不覆盖、清理或回退用户已有的无关修改。

## 接入 Dev 生命周期

复用输入中的正式 `CHG`；没有权威登记源时使用 `CHG-PENDING-001` 并标记临时，不宣称全局唯一。按 [产物协议](../dev-lc/references/artifact-contract.md) 维护 `RUN/EVD/DEFECT/GATE`，按 [状态与阶段门](../dev-lc/references/lifecycle-state-model.md) 分离执行批次、证据和验证门。
...
| 完整验证 | 已批准范围需要完整回归或G5评估 | 是 | `RUN/EVD/DEFECT` 和闭环 `GATE` |
...
至少确认：验证目标与范围、被测版本、适用独立评审结论或明确的项目不适用政策、测试选择、预期来源、环境身份、数据来源与分类、依赖及外部副作用、资源上限、停止条件、清理和恢复方式。每个必选 `Ready TC` 还必须具有可执行人工路径，或关联 `AUT Ready → IMP(kind=test-automation) → BUILD Passed → REV Approved`；人工执行、延期自动化或评审不适用必须有明确项目政策、依据和剩余风险。评审未完成不阻止安全的早期定向验证，但正式G5 Pass不得绕过适用G4评审要求，也不得把只有 `AUT` 规格的用例视为可执行。运行任何命令前读取 [references/execution-safety.md](references/execution-safety.md)。
...
11. 仅使用有效证据聚合适用 `VAL/DVAL`，将执行批次、缺陷、逐目标结果、未验证范围、剩余风险和重新验证条件汇入带 `Suggested/Confirmed` 级别的 `GATE`。
...
`GATE` 评估结果使用 `NotAssessed/Pass/Fail/Blocked/Expired`，确认级别使用 `Suggested/Confirmed`。完整验证和正式G5评估必须形成 `GATE`；定向验证、专项实验或缺陷复测只有在改变阶段门结论或用户明确要求聚合时才形成或更新 `GATE`。门禁重算创建新版本或后继编号，不覆盖旧结论。

`GATE` 至少直接引用适用 `RUN/EVD/DEFECT`，保留逐 `VAL/DVAL` 结果、执行摘要、未验证范围、跳过或隔离项、风险接受、剩余风险、失效与重新验证条件、下一责任方及交接引用。执行数量只帮助阅读，不替代逐目标聚合规则。本 Skill 默认只能给出 `Suggested`，并写 `confirmation_record: null`；输入中存在有权限责任方的明确确认记录时才能记录 `Confirmed`，且 `confirmation_record` 必须保留确认人、带时区时间、非空范围和依据。

需要面向人员展示时，可以由当前 `GATE` 及其引用生成 Markdown、HTML 或表格形式的验证摘要。摘要只是无独立状态的只读视图，必须标明来源 `GATE` 版本，不建立新的正式产物类型，不单独改变结论或确认级别。
...
- 已验证候选的外部交付条件交给项目正式交付责任流程，不在本套件内继续调度。
- 生产事故、恢复或运行风险交给项目正式生产责任流程。
...
只有全部适用条件满足才建议验证完成：计划内 `RUN` 已进入终态；首次失败和全部尝试已保留；结果可追踪到测试、预期、版本和环境；有效 `EVD` 足以支撑结论；失败已分类并交接；清理已验证；证据失效条件已记录；适用 `VAL/DVAL` 已聚合；正式G5范围已有闭环 `GATE`；`GATE` 的评估结果与确认级别分离，并显式汇总执行、缺陷、未验证范围、剩余风险、重新验证条件和下一责任方。
...
- **门禁结论**：适用 `VAL/DVAL`、关联 `RUN/EVD/DEFECT`、逐目标结果、执行摘要、未验证范围、缺失或过期项、风险接受、剩余风险、重新验证条件、下一责任方、`GATE`结果和确认级别。
- **失效与交接**：受影响证据和门禁、恢复条件以及版本化 `Prepared HOF`。

局部请求只输出相关视图，不强制生成完整验证摘要；展示摘要不得脱离来源 `GATE` 成为新的权威产物。
